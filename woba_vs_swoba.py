...
for player in tqdm.tqdm(fg):
    ...
    retro_id = playerid_reverse_lookup([int(player["playerid"])], key_type="fangraphs")["key_retro"].iloc[0]
    # retro_id = retro_id[0]
...
ax = plt.scatter(xpoints, ypoints, s=2).axes
plt.xlabel("wOBA")
plt.ylabel("swOBA")
plt.title("Top 75 qualified wOBA leaders in 2021. wOBA vs swOBA")

texts = []
for i, item in enumerate(labels):
    texts.append(plt.text(xpoints[i], ypoints[i], item, fontsize=6, horizontalalignment='center'))
